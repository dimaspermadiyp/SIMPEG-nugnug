...
class Data_Organisasi extends CI_Controller {
	...
	/*
		***	Controller : data_organisasi.php
		***	by Gede Lumbung
		***	codeigniter
	*/
	...
	public function index()
	{
		if($this->session->userdata('logged_in')!="" && $this->session->userdata('stts')=="administrator")
		{
			...
			$d['tot'] = $offset;
			$tot_hal = $this->db->get_where("tbl_data_organisasi",$kode);
			$config['base_url'] = base_url() . 'data_organisasi/index/';
			...
			$d['data'] = $this->db->get_where("tbl_data_organisasi",$kode,$limit,$offset);
			
			$this->load->view('dashboard_admin/master/header',$d);
			$this->load->view('dashboard_admin/master/data_organisasi/bg_home');
		}
		else
		{
	...
	public function edit()
	{
		if($this->session->userdata('logged_in')!="" && $this->session->userdata('stts')=="administrator")
		{
			...
			$q = $this->db->get_where("tbl_data_organisasi",$id);
			$d = array();
